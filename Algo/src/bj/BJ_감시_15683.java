...
public class BJ_감시_15683 {
	...
	static void dfs(int idx, int[][] parent) {
		...
		switch(c.n) {
			...
			case 2:  //캠의 현재 y, x에서 두 방향으로 x2
				...
				break;
			case 3:  //캠의 현재 y, x에서 두 방향으로 x4 (직각)
				for (int d = 0; d < 4; d++) {
					int[][] child = copyMap(parent);
					//비추는 작업
					watch(c.y, c.x, d, child);
	...
	static void watch(int y, int x, int dir, int[][] array) {
		...
		switch(dir) {
			case 0 : // 좌 
				for (int i = x; i >= 0; i--) {
					if(array[y][i] == 6) break;
					array[y][i] = 9;
				}
				break;
			case 1 : //상
				for (int i = y; i >= 0; i--) {
					if(array[i][x] == 6) break;
					array[i][x] = 9;
				}
				break;
			case 2 : //우
